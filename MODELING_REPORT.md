...
- **Untuned XGBoost.** The project ships an explicit `fallback_params` block used whenever
  CV is infeasible — this is the honest "first iteration": `max_depth 3, eta 0.1,
...
- **Decision threshold:** a trade fires only when the model probability `p ≥
  THRESHOLD_ENTRY = 0.6` (`run_engine`). A high gate trades precision for recall — it is
...
- **Option 5 (by-construction)** is already true of the design and needs nothing: the
  **label** is an explicit Triple-Barrier rule; the **side** is a transparent
  `sign(log_return_5)`; **sizing** is a closed-form Kelly clip `f = clip(λ(2p−1), 0,
  cap)`; and **every feature** has a documented formula, unit and timeframe in
