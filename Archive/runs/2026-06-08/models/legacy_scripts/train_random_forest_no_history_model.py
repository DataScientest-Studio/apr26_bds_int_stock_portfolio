"""Train a Random Forest stock return model without history_days.
...
def evaluate(test_rows: pd.DataFrame, train_rows: pd.DataFrame, split_date: pd.Timestamp) -> pd.DataFrame:
    ...
    return pd.DataFrame(
        [
            {
                "model": "random_forest_no_history",
...
def main() -> None:
    ...
    with (MODEL_DIR / "random_forest_no_history_model.pkl").open("wb") as f:
        pickle.dump({"model": model, "feature_cols": feature_cols}, f)

    metrics.to_csv(MODEL_DIR / "random_forest_no_history_metrics.csv", index=False)
    test_rows[
        ...
        MODEL_DIR / "random_forest_no_history_predictions.csv", index=False
    )
    latest_rankings.to_csv(MODEL_DIR / "random_forest_no_history_latest_rankings.csv", index=False)
    feature_importance.to_csv(MODEL_DIR / "random_forest_no_history_feature_importance.csv", index=False)

    print("Random Forest no-history training complete")
