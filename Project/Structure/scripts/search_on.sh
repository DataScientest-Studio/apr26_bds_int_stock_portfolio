...
# Bake SEARCH_TICKERS into the pane command line rather than relying on tmux's
...
TICKERS_ENV="SEARCH_TICKERS='${SEARCH_TICKERS:-}'"
tmux new-session -d -s "$SESSION" -n run "$TICKERS_ENV bash scripts/_search_run_loop.sh"
...
echo "search loop started: tmux attach -t $SESSION   (windows: run, watch)"
