...
Adapted for the unified app: prices come from the committed daily store (RAW close — corporate
actions deferred, so a split shows as a price cliff; labels and the Explorer page say so).
"""
...
def plot_return_hist(prices: pd.DataFrame):
    ...
    ax.set_xlabel("Daily price change (% per day, raw close-to-close)")
...
def get_price_line_summary(prices: pd.DataFrame, ticker: str) -> dict:
    ...
    total_return_pct = (end["close"] / start["close"] - 1) * 100
    return {
        "ticker": ticker,
...
def plot_price_line(prices: pd.DataFrame, ticker: str):
    ...
    fig, ax = plt.subplots(figsize=(10, 4.5))
    sns.set()
    sns.lineplot(data=data, x="date", y="close", ax=ax, linewidth=1.4, color="steelblue")

    ax.set_xlabel("Trading date")
    ax.set_ylabel("Close (USD, raw — splits not adjusted)")
    ax.set_title(
        f"{ticker} — price path (raw prices; corporate actions deferred)\n"
