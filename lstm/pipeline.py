...
CONFIG = {k: v for k, v in json.loads((ROOT / "config.json").read_text(encoding="utf-8")).items()
...
OVERRIDES_PATH = ROOT / "per_asset_feature_overrides.json"
PROPOSED_PATH = ROOT / "features_proposed.json"


def optional_id_to_name():
    """id -> name for OPTIONAL features (101+) and Claude-PROPOSED features (501+, Stage 3)."""
...
def resolve_manifest(ticker):
    ...
    a run with no overrides is byte-identical to the core-only pipeline."""
    core = list(features.CORE_FEATURE_NAMES)
    if not OVERRIDES_PATH.exists():
        return core
    ids = json.loads(OVERRIDES_PATH.read_text(encoding="utf-8")).get(ticker, [])
    id2name = optional_id_to_name()
    return core + [id2name[i] for i in sorted(ids) if i in id2name]
...
def feature_frame(df, ticker=None):
    ...
    PROPOSED (few, DSL) are always computed fresh, so the cache never goes stale when the Sonnet
...
def hodl_fallback(df, start_idx, end_idx):
    """OOS verdict when the model produced ZERO trades: one long buy-and-hold benchmark
    trade with the same fill/cost model — honestly labeled, never presented as model trades."""
    E0 = CONFIG["INITIAL_CAPITAL_USD"]
    fee = CONFIG["COMMISSION_BPS"] * 1e-4
    slip = CONFIG["SLIPPAGE_BPS"] * 1e-4
    o, c = df["open"].to_numpy(), df["close"].to_numpy()
