"""Train an XGBoost stock return model without history_days.

This uses the same five-year feature pipeline as the Random Forest no-history
experiment. XGBoost is a strong tabular-data model and is a natural next
comparison after Ridge, Random Forest, and the ROCm PyTorch MLP.
...
def main() -> None:
    ...
    model.fit(train_rows[feature_cols], train_rows["target_63d_return"], verbose=False)
    ...
    model.save_model(MODEL_DIR / "xgboost_no_history_model.json")
    with (MODEL_DIR / "xgboost_no_history_model.pkl").open("wb") as f:
        pickle.dump({"model": model, "feature_cols": feature_cols}, f)

    metrics.to_csv(MODEL_DIR / "xgboost_no_history_metrics.csv", index=False)
    test_rows[
        ...
        MODEL_DIR / "xgboost_no_history_predictions.csv", index=False
    )
    latest_rankings.to_csv(MODEL_DIR / "xgboost_no_history_latest_rankings.csv", index=False)
    feature_importance.to_csv(MODEL_DIR / "xgboost_no_history_feature_importance.csv", index=False)

    print("XGBoost no-history training complete")
