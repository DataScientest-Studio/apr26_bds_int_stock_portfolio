...
- **Fixed count / not a filter:** L12 packages **all `×<!--na:universe_size-->503<!--/na-->` frozen per-asset folders** (one per asset, *not* only OOS winners). The [L11](L11_oos_test_eng.md) OOS verdict is an external results report and does **not** change the content or `MODEL_HASH` of any `strategy_<TICKER>.py` — L12 is packaging, not selection.
