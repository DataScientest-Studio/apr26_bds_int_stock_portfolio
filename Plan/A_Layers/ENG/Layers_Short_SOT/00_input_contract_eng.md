...
| Field | Type | Note |
|---|---|---|
| `price_view` | string | the named input price view. **v1 value = `raw_usd_view`** (the DuckDB `VIEW ohlcv_1h` in USD, prices = `raw/10000.0`). Required for repeatability. **One value per dataset** (snapshot manifest), never a per-row column |

The canonical source of the contract is `VIEW ohlcv_1h` (USD) materialized to one
`parquet/<TICKER>/ohlcv.parquet` per ticker (503 files, `zstd`). The contract stays source-independent:
