...
(function () {
  const NODES = [
    ...
    { id: "l1_clv",                  layer: 1, family: "candle",  name: "l1_clv",                  formula: "(2C - H - L) / (H - L)  (H=L → 0)" },
    { id: "l1_wick_imb",             layer: 1, family: "candle",  name: "l1_wick_imb",             formula: "(lw - uw) / (H - L),  uw = H - max(O,C),  lw = min(O,C) - L  (H=L → 0)" },
    ...
    { id: "l2_vol_ratio",            layer: 2, family: "returns", name: "l2_vol_ratio",            formula: "rv_short / max(ε, rv_long)" },
    { id: "l2_mom_ratio",            layer: 2, family: "returns", name: "l2_mom_ratio",            formula: "mom_short / max(ε, |mom_long|)" },
    ...
    { id: "l4_rsi",                  layer: 4, family: "returns", name: "l4_rsi_n",                formula: "100 − 100 / (1 + RS),  RS = avg_gain / max(ε, avg_loss)  (avg_loss=0 → RSI=100)" },
    ...
    { id: "l4_stoch",                layer: 4, family: "range",   name: "l4_stoch_n",              formula: "(C − min_n L) / max(ε, max_n H − min_n L)" },
    { id: "l4_adx",                  layer: 4, family: "range",   name: "l4_adx_n",                formula: "DX = 100·|+DI − −DI| / max(ε, +DI + −DI); ADX = Wilder_smooth(DX, n)" },
    // From volume / TP
    ...
    { id: "l4_mfi",                  layer: 4, family: "volume",  name: "l4_mfi_n",                formula: "100 − 100 / (1 + MR),  MR = pos_flow / max(ε, neg_flow) on TP·V  (neg_flow=0 → MFI=100)" },
    { id: "l4_vwap",                 layer: 4, family: "price",   name: "l4_vwap",                 formula: "Σ TP·V / max(ε, Σ V)" },
