...
- **[`A_Layers/`](A_Layers/) — Main_Pipeline (Pipeline A), layers `L1–L12`.** The **sole build SOT**:
  the S&P 500 ML trading-strategy pipeline (Alpaca → LEAN → DuckDB → snapshot → split → detector →
  features/triple-barrier → quality → Optuna/XGBoost → OOS → per-asset deliverable folder). Start at the canonical SOT folder
...
**Naming rule:** `layer` / `L1–L12` = **A_Layers only**; `feature-stage` / `F0–F14` / `f{stage}_…` (ids on F2–F6) = **B_Features only**. Never mix.

## A_Layers (Main_Pipeline · SOT)

- **[`A_Layers/ENG/Layers_Short_SOT/`](A_Layers/ENG/Layers_Short_SOT/)** — the canonical total SOT: short fact-only files owning every parameter/formula/schema/contract (cross-cutting `00_*` + per-layer L1–L12 + QC-01…QC-11 + 7-X manifest + Output A/B + DoD). Governance + fact-ownership map in [`A_Layers/ENG/Layers_Short_SOT/README.md`](A_Layers/ENG/Layers_Short_SOT/README.md).
...
- [`A_Layers/viz/main_data_flow.html`](A_Layers/viz/main_data_flow.html) — interactive 3D pipeline viz (L1–L12).
...
- Open in any modern browser, or serve over HTTP (`python3 -m http.server`). Deep-links `#1`…`#9`, `#0` (Endproduct), `#setup` (L6), `#dq` (L8), `#L12` (frame the Endproduct layer). Controls: drag = rotate · scroll = zoom · keys `1–9`, `0` (Endproduct), `L` (Anti-leakage) = views.
...
- `layer` / `L1–L12` reserved for `A_Layers`; the feature explanation uses `F0–F14` (feature ids on F2–F6) only.
