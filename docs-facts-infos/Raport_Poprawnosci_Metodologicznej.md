...
> **Status weryfikacji (2026-07-18):** wszystkie referencje plik:linia oraz twierdzenia mechanizmowe tego raportu zostały niezależnie sprawdzone bezpośrednio w kodzie — na gałęzi badawczej `to_give_up_and_show` (commit `d8f920f`) oraz w drzewie `src/` gałęzi `Stable_Presentable_Version`. Wynik: **wszystkie mechanizmy potwierdzone w kodzie**; względem tekstu pierwotnego naniesiono cztery doprecyzowania: (1) linie `848/933` w XGB to asserty fail-closed, a replay do `hi` wykonuje `op_grid_scores` (`pipeline.py:802–803`); (2) choke pointy odczytu OOS znajdują się w entrypointach `run_asset.py`, a rzeczywiste `read_count` uniwersum to XGB = 4 / LSTM = 3 (wartości sprzed re-sealu v5 — aktualne w §Odczyty poniżej); (3) walidator DSL propozycji jest mechanizmem podsystemu LSTM (XGB wybiera z zamkniętego rejestru cech); (4) przy cytowanych dokumentach dopisano gałąź, na której istnieją.
>
