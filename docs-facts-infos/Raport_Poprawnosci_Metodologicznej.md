...
Z tego powodu wersja `v4` przechowuje fallback jako osobny tryb:
...
Okno OOS obejmujące okres `2024–2026` (dokładnie: `2024-01-02` – `2026-05-29`) zostało historycznie odczytane **4 razy dla uniwersum XGB i 3 razy dla uniwersum LSTM**; pojedyncze tickery używane w drylach weryfikacyjnych (np. AAPL) mają odpowiednio do 6 i do 5 odczytów. Dokładna liczba odczytów każdego tickera jest zapisana w ledgerach jako `read_count`.
...
Nie oznacza to leakage wewnątrz aktualnie zamrożonej epoki. Oznacza jednak, że sealed `v4` jest warunkowane wiedzą uzyskaną podczas wcześniejszych odczytów tego samego okresu OOS.
...
Przypadek został udokumentowany w raporcie pilotażowym `docs/FEATURE_PILOT_REPORT.md` (gałąź badawcza).
